...
class ChildViewHolder(itemView: View): RecyclerView.ViewHolder(itemView), RootAdapter.RootItemClickListener {
    ...
    fun setItem(context: Context, childNode: ChildNode) {
        itemView.headerTv.text = childNode.header
        setRootRecyclerView(context, childNode)
    }

    private fun setRootRecyclerView(context: Context, childNode: ChildNode) {
        ...
        val adapter = RootAdapter(childNode.rootNodeList)
